...
import { StyleSheet, FlatList, TouchableOpacity, Pressable, ActivityIndicator, Text, View, SafeAreaView, Alert } from 'react-native';
import { FontAwesome } from '@expo/vector-icons';
import { Link, useRouter, useLocalSearchParams } from 'expo-router';
import { billService, HoaDon } from '@/services/orderapi';
...
export default function OrderScreen() {
  ...
  const checkUserPermissions = async () => {
    try {
      ...
      // Check order view permission
      const canView = await checkPermissionAPI(user.ChucVuNV, 'order.view');

      // Check create/update/delete permissions if can view
      let canCreate = false;
      let canUpdate = false;
      let canDelete = false;

      if (canView) {
        canCreate = await checkPermissionAPI(user.ChucVuNV, 'order.create');
        canUpdate = await checkPermissionAPI(user.ChucVuNV, 'order.update');
        canDelete = await checkPermissionAPI(user.ChucVuNV, 'order.delete');
      }
  ...
  const handleOrderPress = (order: HoaDon) => {
    if (!permissions.canView) {
      Alert.alert(
        "Thông báo",
        "Bạn không có quyền xem chi tiết hóa đơn.",
        [{ text: "Đã hiểu" }]
      );
      return;
    }
  ...
  const handleLongPress = (id: string) => {
    if (!permissions.canDelete) {
      Alert.alert(
        "Thông báo",
        "Bạn không có quyền xóa hóa đơn.",
        [{ text: "Đã hiểu" }]
      );
      return;
    }
  ...
  return (
    <View style={styles.container}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              ...
              onLongPress={() => permissions.canDelete && handleLongPress(item.idhoadon.toString())}
            >
      ...
      {/* Tạo order button - chỉ hiển thị nếu có quyền tạo */}
      {permissions.canCreate && (
        <TouchableOpacity 
          style={styles.createOrderButton} 
          onPress={() => router.push('../createorder/CreateOrderScreen')}
        >
